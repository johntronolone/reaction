...
export default async function addCartItems(context, currentItems, inputItems, options = {}) {
  ...
  const promises = inputItems.map(async (inputItem) => {
    ...
    const cartItem = {
      ...
      shippingOverride: catalogProduct.shippingOverride,
      imageURLs: ((chosenVariant.primaryImage && chosenVariant.primaryImage.URLs)? chosenVariant.primaryImage.URLs : catalogProduct.primaryImage.URLs)
    };
