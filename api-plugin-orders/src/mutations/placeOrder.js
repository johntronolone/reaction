...
export default async function placeOrder(context, input) {
  ...
  const finalFulfillmentGroups = await Promise.all(fulfillmentGroups.map(async (inputGroup) => {
    ...
    if (group.items) {
      group.items.forEach(item => {
        if (item.shippingOverride) {
          item.shippingOverride.forEach(entry => {
            if (entry.state == group.address.region) { 
              orderTotal += entry.surcharge * item.quantity;
              shippingTotal += entry.surcharge * item.quantity;
            }
          });
        }
      });
    ...
    //console.log('group from finalFulfillmentGroups');
    //console.log({group});

    if (shippingTotal) { 
      group.invoice.shipping = shippingTotal;
      group.invoice.total += shippingTotal;
    }
