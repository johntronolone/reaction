...
export default async function getDataForOrderEmailDefault(context, { order }) {
  ...
  const adjustedOrderGroups = await Promise.all(order.shipping.map(async (group) => {
    ...
    //console.log('in api-plugin-orders (20201-05-12):');
    //console.log({items});
    ...
    items = items.map((item) => ({
      ...
      productImage: item.imageURLs && 'https://backend.patchamp.com' + item.imageURLs.large,
      variantImage: item.imageURLs && 'https://backend.patchamp.com' + item.imageURLs.large,
      imageURLs: item.imageURLs
    }));

    //console.log('items after edit in api-plugin-orders (2021-05-12):');
    //console.log({items});
    return { ...group, items };
  }));
