...
export default async function updateGroupTotals(context, {
  ...
  //console.log({cart});
  //console.log({group});
  //console.log({groupSurchargeTotal});
      ...
      if (item.shippingOverride) {
        item.shippingOverride.forEach(entry => {
          if (entry.state == group.address.region) { 
            //console.log(entry);
            newSurcharge += entry.surcharge * item.quantity;
          }
        });
      }
    });
  ...
  if (expectedGroupTotal) {
    ...
    // expectedTotal is total incluing discounts...
    ...
    compareExpectedAndActualTotals(group.invoice.total + newSurcharge,  expectedTotal);
  }
